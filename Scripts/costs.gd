...
static func token(chain: String) -> String:
	return "MON" if is_monad(chain) else "SOL"
...
## Creating one table. On Monad this is a flat protocol fee regardless of size.
...
## Formats an amount with enough places to be meaningful on either chain:
## a Solana figure needs four, a Monad figure looks absurd with them.
...
## One line comparing both chains, so the choice can be made on numbers.
static func comparison() -> String:
	return (
		"A check-in is about %s on Solana, or %s on Monad. Bigger writes cost more."
		% [format("sol", write_row("sol")), format("mon", write_row("mon"))]
	)
