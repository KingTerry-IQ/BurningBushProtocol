...
## The HTTP status that went with it, or 0 when the host was never reached.
##
## The message alone is not enough to act on: a caller that wants to treat one
## particular refusal differently — a 409 saying the host is busy, say — should
## not have to match on the host's wording to recognise it.
var last_code: int = 0
